...
- **Inbox** — reach users on Telegram, Slack, Discord, email, and WhatsApp from one agent
- **Scheduled delivery** — cron-driven agents that ship their reply to Slack, email, Discord, Telegram, or WhatsApp
- **Self-improving skills** — operator-approved tools synthesised from the trajectories the agent has already run
- **Memory** — session history, semantic memory, per-user facts, and a rolling LLM-summarised user profile
- **Providers** — Anthropic, OpenAI, Gemini, and OpenRouter (300+ models behind one key)
- **Deploy** — one command to Docker, Railway, Fly, Modal (scale-to-zero), or Daytona
