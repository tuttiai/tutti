...
| `memory` | No | Long-term memory (`{ semantic?, user_memory?, user_model? }`). See [Memory](#memory). |
...
Tutti has four kinds of memory, each configured separately:
...
| **User memory** | All sessions for an end-user, across agents (per-fact) | Postgres | `agent.memory.user_memory` |
| **User model** | One rolling LLM-summarised profile per end-user | In-memory (Postgres planned) | `agent.memory.user_model` |
...
**User model** is a rolling LLM-summarised profile (`summary`, `preferences`, `ongoing_projects`) that complements (not replaces) `user_memory`. The runtime injects the profile into the system prompt above the per-fact list at run start, and a periodic consolidation pass re-summarises the profile from accumulated `UserMemory` entries every `every_n_turns` turns. Consolidation is fire-and-forget and never crashes a run.
...
```ts
{
  assistant: {
    memory: {
      ...
      user_model: { enabled: true, every_n_turns: 20 },
    },
  },
}
```
...
All four providers support streaming: Anthropic (message stream events), OpenAI (delta chunks), Gemini (content stream), and OpenRouter (delta chunks via the OpenAI-compatible SSE protocol).
