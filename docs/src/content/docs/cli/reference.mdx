...
- **Running agents** — [`run`](#tutti-ai-run-score), [`serve`](#tutti-ai-serve-score), [`studio`](#tutti-ai-studio-score), [`inbox start`](#tutti-ai-inbox-start-score), [`resume`](#tutti-ai-resume-session-id), [`replay`](#tutti-ai-replay-session-id)
- **Deployment** — [`deploy`](#tutti-ai-deploy), [`deploy verify-hibernate`](#tutti-ai-deploy-verify-hibernate)
...
### `tutti-ai inbox start [score]`

Start the inbound messaging orchestrator — wires platform adapters declared on `score.inbox.adapters` to the agent named in `score.inbox.agent`, and runs until SIGINT/SIGTERM. Telegram, Slack, Discord, email, and WhatsApp ship in v0.25.0; Twitter follows once its voice's shared client lands.

```bash
tutti-ai inbox start                    # defaults to ./tutti.score.ts
tutti-ai inbox start ./path/to/score.ts # custom path
```
...
- Loads the score, builds adapters from `score.inbox.adapters`, constructs `TuttiInbox`, and calls `.start()`.
- Each adapter logs a "connected" line on start.
- `inbox:message_received` / `inbox:message_replied` / `inbox:message_blocked` / `inbox:error` events stream through the runtime's event bus.
- SIGINT/SIGTERM trigger parallel adapter teardown — every platform connection (Telegram poll, Slack Socket Mode socket, Discord Gateway, IMAP IDLE, WhatsApp Fastify webhook server) is closed cleanly before exit.
- The `buildAdapter` switch is exhaustively typed — adding a new platform to the `InboxAdapterConfig` union without updating the CLI fails the typecheck.
...
| Slack | `SLACK_BOT_TOKEN` (`xoxb-…`) + `SLACK_APP_TOKEN` (`xapp-…` for Socket Mode) |
| Discord | `DISCORD_BOT_TOKEN` |
| Email | `TUTTI_EMAIL_PASSWORD` (or `TUTTI_EMAIL_IMAP_PASSWORD` + `TUTTI_EMAIL_SMTP_PASSWORD`) |
| WhatsApp | `WHATSAPP_ACCESS_TOKEN` + `WHATSAPP_VERIFY_TOKEN` + `WHATSAPP_APP_SECRET` (+ a public tunnel pointing at port 3848) |

See the [Inbox guide](/guides/inbox) for adapter-specific setup, identity linking, and per-platform safety defaults.
...
Bundle your score and ship it to a hosting platform. The CLI ships bundlers for five targets — `docker`, `railway`, `fly`, `modal`, `daytona` — generated from a `DeployConfig` block on the agent.
...
tutti-ai deploy --target modal                 # bundle + modal deploy (scale-to-zero)
tutti-ai deploy --target daytona               # bundle + daytona create + ssh
...
| Flag | Default | Description |
| ---- | ------- | ----------- |
| `--target <platform>` | (required) | One of `docker`, `railway`, `fly`, `modal`, `daytona`. `cloudflare` is accepted on the score's `DeployTarget` union but the CLI errors loudly — its bundler is a follow-up. |
...
Roll back to the previous deploy. Dispatches to the platform's equivalent (`railway rollback`, `fly releases rollback <prev>`, `docker compose pull && up -d` for the prior tag). Modal redeploys are functions, not slot-based releases, so rollback is reported as not applicable on Modal.
...
Check a score against the serverless-hibernation contract before a `--target modal` (or any scale-to-zero) deploy. Flags long-lived in-memory stores and background timers outside the scheduler — anything that wouldn't survive the next cold start.

```bash
tutti-ai deploy verify-hibernate
tutti-ai deploy verify-hibernate -s ./path/to/score.ts
```
...
Exits non-zero on a finding, so it composes into CI as a deploy gate.
...
v0.26 adds optional `deliver` + `deliver_format` to a schedule block — when set, the scheduler dispatches the agent's final text reply to a platform after every successful run. Per-platform paths: Slack `chat.postMessage`, Discord `channels.fetch(...).send(...)`, Telegram `sendMessage` (with `parse_mode: "MarkdownV2"` when `deliver_format === "markdown"`), email `EmailClientWrapper.send`, WhatsApp `sendText` (markdown stripped before send — `logger.warn` makes the loss visible).
...
`ScheduleDeliveryTarget` is a discriminated union over `platform` and carries addressing only — no tokens. Credentials are read from `SecretsManager` per the existing per-voice convention (`SLACK_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `TELEGRAM_BOT_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `TUTTI_EMAIL_SMTP_*` / `TUTTI_EMAIL_FROM`). Each voice's `forToken` / `forKey` cache means a delivery reuses whichever wrapper the agent's matching voice already initialised. Delivery failures are absorbed in the engine — they never crash the schedule timer — and surface as `schedule:delivery_failed`; success fires `schedule:delivered` with the dispatched payload length.
...
Review and approve skill candidates synthesised from agent trajectories. Backed by the `@tuttiai/skills` store; today the CLI uses a process-scoped in-memory store, so review sessions are most useful alongside a long-running `tutti-ai serve` or `tutti-ai run --watch` that emits proposals.

```bash
tutti-ai skills list                          # approved skills
tutti-ai skills proposed                      # pending candidates
tutti-ai skills review                        # walk every pending candidate
tutti-ai skills review <candidate-id>         # review one
tutti-ai skills reject <candidate-id> --reason "..."
```
...
| Flag | Default | Description |
| ---- | ------- | ----------- |
| `-s, --score <path>` | `./tutti.score.ts` | Score file path. |
| `--by <name>` | unset | Reviewer identifier (stored as `reviewed_by` on the audit record). |

The interactive review screen shows the candidate's name, agent (resolved from its first evidence trajectory), input/output signature, constituent tools, computed `is_destructive` flag, the union of `required_permissions`, and a sample of evidence trajectories. The action prompt accepts:

- `a` — approve with the synthesiser's description as the system prompt
- `e` — edit the system prompt, then approve
- `r` — reject (prompts for a free-text reason)
- `s` — skip to the next candidate

**Constituent-tool resolution.** Approval computes `is_destructive` as the union of each constituent tool's `destructive` flag and `required_permissions` as the union of the owning voices' permissions. If any constituent tool no longer resolves against the loaded score (typically because the voice was removed since the candidate was proposed), approval is blocked with an actionable error pointing at `tutti-ai skills reject`. Reviewers should add the voice back to an agent in the score or reject the candidate.
