...
The same `DiscordClientWrapper` powers [`@tuttiai/inbox`](/guides/inbox/)'s Discord adapter. Discord's Gateway API allows only one bot session per token, so the inbox adapter and the voice share a single Client via `DiscordClientWrapper.forToken(token)`. Configure the inbox adapter in your score:
