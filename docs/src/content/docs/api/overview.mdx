...
Smart model routing — meta-provider that picks the cheapest configured model per turn based on task difficulty, the agent's destructive-tool count, and the active token budget. See the [Smart Routing guide](/guides/smart-routing) for usage and policy choices.
