...
- **@tuttiai/email** — new voice. Three tools: `send_email` and `send_reply` (both `destructive: true`) + `list_inbox` (read-only). Backed by `imapflow` 1.0.171 (IMAP IDLE — server pushes new mail, no polling), `nodemailer` 7.0.5 (SMTP), and `mailparser` 3.7.4 (RFC 5322 → structured). Exports `EmailClientWrapper` with a `forKey(host:port:user)` shared cache so the voice's outbound tools and the `@tuttiai/inbox` email adapter share one IMAP connection and one SMTP transporter. Passwords NEVER inline — resolved via `SecretsManager.optional` from `TUTTI_EMAIL_IMAP_PASSWORD` / `TUTTI_EMAIL_SMTP_PASSWORD` (or shared `TUTTI_EMAIL_PASSWORD`).
