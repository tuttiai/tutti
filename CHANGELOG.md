...
Clears all five GitHub Security Advisories acknowledged in `SECURITY-NOTES-v0.26.0.md` (four high, one moderate) plus a `devalue` DoS advisory surfaced in the docs site after v0.26.0 shipped — 14 open Dependabot alerts in total. `npm audit --audit-level=high` is clean across the root workspace and `docs/`.
...
| `protobufjs` | `<=7.5.5` | high / moderate | [GHSA-66ff-xgx4-vchm](https://github.com/advisories/GHSA-66ff-xgx4-vchm), [GHSA-2pr8-phx7-x9h3](https://github.com/advisories/GHSA-2pr8-phx7-x9h3), [GHSA-fx83-v9x8-x52w](https://github.com/advisories/GHSA-fx83-v9x8-x52w), [GHSA-75px-5xx7-5xc7](https://github.com/advisories/GHSA-75px-5xx7-5xc7), [GHSA-685m-2w69-288q](https://github.com/advisories/GHSA-685m-2w69-288q), [GHSA-jvwf-75h9-cwgg](https://github.com/advisories/GHSA-jvwf-75h9-cwgg), [GHSA-2qrj-frcc-jp5g](https://github.com/advisories/GHSA-2qrj-frcc-jp5g) — code injection, DoS, prototype pollution |
...
- **@tuttiai/core 0.23.0 → 0.23.1** (patch)
  - `@opentelemetry/sdk-node` `^0.214.0` → `^0.218.0`
  - `@opentelemetry/auto-instrumentations-node` `^0.72.0` → `^0.76.0`
  - `@opentelemetry/exporter-trace-otlp-http` `^0.214.0` → `^0.218.0`
- **@tuttiai/telemetry 0.4.0 → 0.4.1** (patch)
  - `@opentelemetry/otlp-transformer` `^0.214.0` → `^0.218.0` (keeps the OpenTelemetry contrib set on a single coherent version after the core SDK upgrade)
- **docs/** lockfile-only — lifts `astro` → `devalue@5.8.1` via `npm audit fix`. No version bump.

### Compatibility

`SECURITY-NOTES-v0.26.0.md` forecast the OpenTelemetry SDK 0.214 → 0.218 upgrade as a breaking change to exporter-configuration shape. In practice, `packages/core/src/telemetry-setup.ts`'s use of `NodeSDK`, `getNodeAutoInstrumentations`, and `OTLPTraceExporter` continues to compile and pass the existing telemetry tests against 0.218.0 / 0.76.0 unchanged. No tutti public-API change; no migration required.

### Removed

`SECURITY-NOTES-v0.26.0.md` — the advisories it tracked are now cleared. As stated in that file: "This document will be deleted when v0.26.1 ships clean."

## v0.26.0 — Self-improving skills, serverless runtime, scheduled delivery. (2026-05-17)

### New

#### Self-improving skills

- **@tuttiai/skills** — new package (0.1.0). Storage and operator-review primitives for agent-callable skills synthesised from observed trajectories. Ships the `SkillStore` interface and `InMemorySkillStore` reference: trajectory recording (`recordTrajectory` / `listTrajectories`), candidate proposals (`proposeCandidate` / `listCandidates`), and operator review (`approveCandidate` / `rejectCandidate` / `listSkills`). On approval the candidate is removed from the candidate list and re-emerges as a `Skill` with the same id; rejected candidates are kept for audit with `status: "rejected"`. Optionally accepts a structural `SkillEventBus` and emits `skill:candidate_proposed`, `skill:approved`, `skill:rejected`. `SkillSignature` keeps input/output as Zod refs — JSON Schema export is a follow-up when a persistent backend lands.
- **@tuttiai/core** — closes the trajectory → proposal → review → invocation loop. `TrajectoryObserver` records every successful run's `(tool, input_hash, succeeded, duration_ms)` tuples and forwards eligible trajectories to the proposer. `SkillProposer` asks the score's LLM to synthesise a `SkillCandidate` from a trajectory window, deduplicates against prior candidates, and persists. `SkillExecutor` projects approved skills into callable `Tool`s so the agent invokes them like a voice tool; permissions for a skill are the union of constituent tools' permissions, checked at run start. `TuttiRuntime` constructs both helpers when `score.skills.enabled` is true and a `skillStore` is supplied — runtime cost is zero when off. `AgentRunner` gains a per-run trajectory audit and emits `skill:invoked` and the run-end `skill:trajectory_observed` event. New public exports: `TrajectoryObserver`, `SkillProposer`, `SkillExecutor`, `hashToolInput`, and the matching option types.
- **@tuttiai/types** — `SkillsConfig` added to `ScoreConfig` (`enabled`, `auto_propose_threshold`). Five new event variants: `skill:candidate_proposed`, `skill:approved`, `skill:rejected`, `skill:invoked`, `skill:trajectory_observed`. No behaviour change for existing event subscribers.
- **@tuttiai/cli** — new `tutti-ai skills` command group: `list` (approved skills, newest first), `proposed` (pending candidates), `review [candidate-id]` (interactive walk-through, prompts for approve / edit-then-approve / reject / skip via enquirer), and `reject <candidate-id> --reason "..."` (non-interactive). Constituent-tool resolution lives in the CLI: it loads the score, walks each agent's voices, computes the `is_destructive` and `required_permissions` unions, and passes both to `SkillStore.approveCandidate` via `ApproveCandidateOptions` — no public-interface change to `@tuttiai/skills`. Approval is blocked with an actionable error when a constituent tool no longer resolves; the message points at `skills reject` as the audit-trail-preserving fallback. Pure render functions live in `commands/skills-render.ts` (fully covered); the enquirer-driven REPL in `commands/skills.ts` is excluded from coverage as `commands/interrupts.ts` already is.
- **examples/self-improving-skills** — score with `skills.enabled: true` plus a README walkthrough of `tutti-ai skills review`.
...
- **@tuttiai/deploy / @tuttiai/types / @tuttiai/cli** — `DeployTarget` union extended to `"docker" | "cloudflare" | "railway" | "fly" | "modal" | "daytona"` (additive: existing targets unchanged). The Modal bundle (`generateModalBundle`) emits `modal_app.py`, a copy of the score at `tutti.score.ts`, `.env.modal.example`, and an executable `deploy.sh`. The generated `modal_app.py` runs the Tutti Node server inside `node:20-bookworm-slim` (Debian's apt `nodejs` is Node 18 and too old), installs `tutti-ai@latest` + `@tuttiai/cli@latest` (constant `CLI_VERSION` pinnable before `modal deploy`), declares each `manifest.secrets` entry as `modal.Secret.from_name("tutti-<lowercase>")`, surfaces `manifest.env` as the function's `env=` dict, and exposes via `@modal.web_server(port=3000, startup_timeout=120)`. `manifest.scale.memory` (`"512mb"` / `"2gb"`) is converted to the integer Modal expects on `memory=`, defaulting to 2048. The CLI's `deploy --target modal` is wired through `buildDeployPlan` (now accepts an optional `scoreContents` arg since Modal embeds the score); `deploy status / logs` dispatch to `modal app list` / `modal app logs <name>`; `deploy rollback` is reported as not applicable (Modal redeploys).
- **@tuttiai/deploy** — Daytona added as a target (`tutti-ai deploy --target daytona`). Generates a devcontainer bundle (`.devcontainer/devcontainer.json`, `.daytona/snapshots.yaml`, `.gitignore`, `daytona.sh`) for an always-warm agent dev environment. Forwards port 3000, pins `tutti-ai@latest`, runs `daytona create --no-ide` followed by `daytona ssh -- tutti-ai serve …`. The `.daytona/snapshots.yaml` field names (`idle_minutes_until_hibernate`, `auto_resume`) are not yet verified against the current Daytona schema — ships with a TODO banner; hibernation may need to move into `daytona sandbox create` flags in a follow-up.
- **@tuttiai/cli** — new `tutti-ai deploy verify-hibernate` subcommand. Checks a score against the serverless-hibernation contract (no long-lived in-memory stores, no background timers outside the scheduler) so a `--target modal` deploy doesn't silently lose state between invocations.
- **ci / examples/marketing-agent** — nightly + on-push Modal smoke test (`.github/workflows/modal-smoke.yml`). Generates the Modal bundle from the marketing-agent example, provisions the matching Modal secret with `--force`, runs `modal deploy modal_app.py`, resolves the `@modal.web_server` URL via `modal.Function.from_name("tutti-smoke", "serve").get_web_url()` (falls back to the legacy `.web_url`), polls `GET /health` for up to 240s — the blind 30s sleep in the original spec was a flake risk against `node:20-bookworm-slim` cold starts — and always runs `modal app stop tutti-smoke` so no app leaks into the next nightly window. Skips cleanly when `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET` / `ANTHROPIC_API_KEY` are unset, so PRs from forks never fail this job. PRs touching `packages/deploy/src/targets/modal.ts` get auto-labelled `deploy-smoke` by `.github/workflows/deploy-smoke-label.yml` (uses `pull_request_target` for the write-scoped label token; intentionally does *not* check out the PR head — labelling only), which retriggers the smoke. `packages/deploy/README.md` documents the local setup, the three required repo secrets, and the CI flow.

#### Scheduled delivery

- **@tuttiai/types / @tuttiai/core** — `AgentScheduleConfig` gains optional `deliver` (slack / discord / telegram / email / whatsapp) and `deliver_format` (`text | markdown`). `ScheduleDeliveryTarget` exported as a discriminated union for downstream dispatch. After each successful `ScheduledAgentRunner` run, the engine dispatches the agent's text output to the configured `deliver` target via the new `scheduler/dispatch.ts` module (`deliverScheduleResult`). Per-platform paths: Slack `chat.postMessage`, Discord `channels.fetch(...).send(...)`, Telegram `sendMessage` (with `parse_mode: "MarkdownV2"` when `deliver_format === "markdown"`), email `EmailClientWrapper.send` (subject defaults to `<agent_name> run — <ISO date>` when `subject` is unset), WhatsApp `sendText` (markdown stripped before send because WhatsApp doesn't render it — emits a `logger.warn` so the loss is visible).
- **@tuttiai/core** — Voices stay optional peer deps — dynamic-imported at delivery time; a missing package throws a clear `Install @tuttiai/<platform> …` remediation error. Credentials are read from `SecretsManager` per the existing per-voice env-var convention (`SLACK_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `TELEGRAM_BOT_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `TUTTI_EMAIL_SMTP_*` / `TUTTI_EMAIL_FROM`) — `ScheduleDeliveryTarget` carries addressing only, no tokens. Each voice's shared `forToken` / `forKey` cache means a delivery reuses whichever wrapper the agent's matching voice already initialised. WhatsApp is the lone exception: its wrapper constructor allocates a Fastify webhook server, so the dispatcher only uses a cached wrapper and surfaces a precise "voice not active" error when one isn't present. Delivery failures are absorbed in `engine.executeRun` — never propagate up to crash the schedule timer — and surface as the `schedule:delivery_failed` event; success fires `schedule:delivered` with `chars` reporting the dispatched payload length. New public exports: `deliverScheduleResult`, `deliveryTargetSummary`, `stripMarkdown`, `DeliverOptions`.
- **examples/daily-standup-to-slack** and **examples/weekly-revenue-report-to-email** — two end-to-end examples demonstrating the slack and email delivery paths.
...
Fully additive. No breaking changes. The `skills` block is opt-in; the `deliver` field on schedules is optional; new deploy targets sit alongside the existing four. Existing scores upgrade without edits.

### Known advisories

Five npm-audit advisories (4 high, 1 moderate) remain open in transitive dependencies of `@tuttiai/telemetry` (`@opentelemetry/sdk-node`, `@opentelemetry/exporter-prometheus`, `@opentelemetry/auto-instrumentations-node`, `protobufjs`, `@protobufjs/utf8`). They are not introduced by v0.26.0 — the OpenTelemetry chain was already in place. Clearing them requires a breaking-change upgrade of the OpenTelemetry SDK, scheduled for v0.26.1. See [SECURITY-NOTES-v0.26.0.md](SECURITY-NOTES-v0.26.0.md) for exposure analysis and mitigations.
...
### New
- **@tuttiai/core / @tuttiai/types** — dialectic user model (`AgentConfig.memory.user_model`). New `UserProfile` document — one rolling LLM-summarised profile per `user_id` (`summary`, `preferences`, `ongoing_projects`) — sits alongside the existing per-fact `user_memory` store rather than replacing it. Pluggable `UserModelStore` interface with `InMemoryUserModelStore` shipping in this release; the same factory pattern used for `UserMemoryStore` lets a Postgres-backed implementation land later behind the same surface. The new `UserModelConsolidator` runs an LLM consolidation pass every `every_n_turns` turns (default 20), reading recent entries from the agent's `UserMemoryStore` (NOT `SemanticMemoryStore`, which is agent-keyed) to produce a refreshed JSON profile. Output validated against `UserProfileWritableSchema`; parse failures or schema violations preserve the previous profile. Consolidation is fire-and-forget — failures are caught, logged, and never crash the run. Runtime injects the profile into the system prompt before the existing "What I remember about you:" block. New `user_model:consolidated` event for telemetry/sync. New `TuttiRuntime.setUserModelStore()` test seam mirrors `setUserMemoryStore`. Bootstrap-empty profiles do not inject (no dead weight on brand-new users) and are skipped by the consolidator (no LLM cost). Consolidation prompt explicitly forbids sensitive data (passwords, API keys, SSNs, payment-card numbers, government identifiers).
- **examples/user-model** — 25-turn end-to-end demonstration with a mock LLM provider. Shows the consolidator triggering at every-5-turn cadence, the profile evolving as the conversation progresses, and the final dump.
- **@tuttiai/core** — `OpenRouterProvider` for OpenAI-compatible access to 300+ models across providers (Anthropic, OpenAI, Google, Meta, Mistral, …) through one API key. Reuses the OpenAI SDK with a custom `baseURL` (default `https://openrouter.ai/api/v1`) plus optional `HTTP-Referer` / `X-Title` attribution headers, so streaming, tool calling and SSE parsing inherit the existing tested code paths. Cost is reported inline via OpenRouter's `usage: { include: true }` extension and surfaced on `ChatResponse.usage.cost_usd` and `StreamChunk.usage.cost_usd` — no second `/generation` round trip. Adds `route?: "fallback"` and `models?: string[]` for OpenRouter's cross-provider failover. API key resolves via `SecretsManager.optional("OPENROUTER_API_KEY")`. 401 → `AuthenticationError`, 429 → `RateLimitError` (with `Retry-After` parsing), other HTTP errors → `ProviderError` with `status` preserved on the error context. Per-provider unit tests landed for all four providers (anthropic / openai / gemini / openrouter) with the SDK mocked at module boundary; this also closes the prior gap where `src/providers/**` had zero unit coverage and was hidden behind the vitest coverage exclude.
...
- **@tuttiai/types** — three new memory event variants: `memory:write`, `memory:read`, and `memory:delete` (with `reason: "explicit" | "lru_eviction"`). Both the curated agent tools and `ToolContext.memory` helpers emit them.
