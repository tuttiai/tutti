...
### New
- **@tuttiai/core** — scheduled-run delivery is now wired end-to-end. After each successful `ScheduledAgentRunner` run, the engine dispatches the agent's text output to the configured `deliver` target via the new `scheduler/dispatch.ts` module (`deliverScheduleResult`). Per-platform send paths: Slack `chat.postMessage`, Discord `channels.fetch(...).send(...)`, Telegram `sendMessage` (with `parse_mode: "MarkdownV2"` when `deliver_format === "markdown"`), email `EmailClientWrapper.send` (subject defaults to `<agent_name> run — <ISO date>` when `ScheduleDeliveryTarget.email.subject` is unset), WhatsApp `sendText` (markdown stripped before send because WhatsApp doesn't render it — emits a `logger.warn` so the loss is visible). Voices stay optional peer deps — dynamic-imported at delivery time; a missing package throws a clear `Install @tuttiai/<platform> …` remediation error. Credentials are read from `SecretsManager` per the existing per-voice env-var convention (`SLACK_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `TELEGRAM_BOT_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `TUTTI_EMAIL_SMTP_*` / `TUTTI_EMAIL_FROM`) — `ScheduleDeliveryTarget` continues to carry addressing only, no tokens. Each voice's shared `forToken` / `forKey` cache means a delivery reuses whichever wrapper the agent's matching voice already initialised. WhatsApp is the lone exception: because its wrapper constructor allocates a Fastify webhook server, the dispatcher only uses a cached wrapper and surfaces a precise "voice not active" error when one isn't present. Delivery failures are absorbed in `engine.executeRun` — never propagate up to crash the schedule timer — and surface as the existing `schedule:delivery_failed` event (the dual `schedule:delivered` event fires on success, with `chars` reporting the dispatched payload length). New public exports from `@tuttiai/core`: `deliverScheduleResult`, `deliveryTargetSummary`, `stripMarkdown`, `DeliverOptions`.
- **ci / examples/marketing-agent** — nightly + on-push Modal smoke test (`.github/workflows/modal-smoke.yml`). Generates the Modal bundle from `examples/marketing-agent/tutti.score.ts` (now carries a `deploy: { target: "modal", name: "tutti-smoke", secrets: ["ANTHROPIC_API_KEY"], scale: { memory: "512mb" } }` block), provisions the matching Modal secret with `--force`, runs `modal deploy modal_app.py`, resolves the `@modal.web_server` URL via `modal.Function.from_name("tutti-smoke", "serve").get_web_url()` (falls back to the legacy `.web_url` attribute on older Modal SDK versions), polls `GET /health` for up to 240s — the blind 30s sleep in the original spec was a flake risk against `node:20-bookworm-slim` cold starts — and always runs `modal app stop tutti-smoke` so no app leaks into the next nightly window. Skips cleanly (job ends green) when `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET` / `ANTHROPIC_API_KEY` are unset, so PRs from forks never fail this job. PRs touching `packages/deploy/src/targets/modal.ts` get auto-labeled `deploy-smoke` by `.github/workflows/deploy-smoke-label.yml` (uses `pull_request_target` for the write-scoped label token; intentionally does *not* check out the PR head — labelling only), which retriggers the smoke. `packages/deploy/README.md` documents the local setup, the three required repo secrets, and the CI flow.
- **@tuttiai/deploy / @tuttiai/types / @tuttiai/cli** — Modal added as a deploy target. `DeployTarget` union extended to `"docker" | "cloudflare" | "railway" | "fly" | "modal" | "daytona"` (additive: existing targets unchanged). `daytona` is a placeholder — present in the union so future work can plug in a bundler without another type change, but the CLI rejects it with the same "not yet wired up" error it has long used for `cloudflare`. The Modal bundle (`generateModalBundle`) emits `modal_app.py`, a copy of the score at `tutti.score.ts`, `.env.modal.example`, and an executable `deploy.sh`. The generated `modal_app.py` runs the Tutti Node server inside a `node:20-bookworm-slim` image (Debian's apt `nodejs` package is Node 18 and too old), installs `tutti-ai@latest` + `@tuttiai/cli@latest` (constant `CLI_VERSION` pinnable by the user before `modal deploy`), declares each `manifest.secrets` entry as `modal.Secret.from_name("tutti-<lowercase-name>")`, surfaces plaintext `manifest.env` as the function's `env=` dict, and exposes the Node server via `@modal.web_server(port=3000, startup_timeout=120)` so `/messages` is routed without a Python ASGI wrapper. `manifest.scale.memory` (`"512mb"` / `"2gb"`) is converted to the integer Modal expects on `memory=`, defaulting to 2048 when unset. The CLI's `deploy --target modal` is wired through `buildDeployPlan` (now accepts an optional `scoreContents` arg since Modal embeds the score in the bundle); `deploy status / logs` dispatch to `modal app list` / `modal app logs <name>`, and `deploy rollback` is reported as not applicable (Modal redeploys instead of rolling back).
- **@tuttiai/skills** — new package: storage and operator-review primitives for agent-callable skills synthesised from observed trajectories. v0.1 ships the `SkillStore` interface and `InMemorySkillStore` reference implementation, covering trajectory recording (`recordTrajectory`/`listTrajectories`), candidate proposals (`proposeCandidate`/`listCandidates`), and operator review (`approveCandidate`/`rejectCandidate`/`listSkills`). On approval the candidate is removed from the candidate list and re-emerges as a `Skill` with the same id (so downstream references stay valid); rejected candidates are kept for audit with `status: "rejected"`. Optionally accepts an `EventBus` and emits `skill:candidate_proposed`, `skill:approved`, `skill:rejected` on the matching operations. `SkillSignature` keeps input/output as Zod refs (matching repo convention) — JSON Schema export is a follow-up when a persistent backend lands. Candidate synthesiser and runtime tool-adapter are out of scope for v0.1.
- **@tuttiai/types** — three new event variants: `skill:candidate_proposed` (candidate id, name suggestion, evidence count), `skill:approved` (skill id, name, optional `reviewed_by`), `skill:rejected` (candidate id, optional `reviewed_by`, optional `reason`). Consumed by `@tuttiai/skills`; no behaviour change for existing event subscribers.
- **@tuttiai/cli** — new `tutti-ai skills` command group: `list` (approved skills, newest first), `proposed` (pending candidates), `review [candidate-id]` (interactive walk-through, prompts for approve / edit-then-approve / reject / skip via enquirer), and `reject <candidate-id> --reason "..."` (non-interactive). Constituent-tool resolution lives in the CLI: it loads the score, walks each agent's voices, computes the `is_destructive` union (across `Tool.destructive`) and the `required_permissions` union (across owning `Voice`s), and passes both to `SkillStore.approveCandidate` via the existing `ApproveCandidateOptions` — no public-interface change to `@tuttiai/skills`. Approval is blocked with a clear, actionable error when a constituent tool no longer resolves (typically because the voice was removed from the score after the candidate was proposed); the message points at `skills reject` as the audit-trail-preserving fallback. Pure render functions live in `commands/skills-render.ts` (fully covered); the score-loading + enquirer-driven REPL in `commands/skills.ts` is excluded from coverage in the same manner as `commands/interrupts.ts`. Today the CLI uses a process-scoped `InMemorySkillStore` and prints a one-line banner on each invocation making the lack of cross-invocation persistence explicit — a persistent store (HTTP-via-`serve` or a Postgres-backed `SkillStore`) is the planned next step.
